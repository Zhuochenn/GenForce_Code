...
class SequentialDataset(Dataset):  
    def __init__(self, direction, force_dir, img_dir, modulus_dir, depth_dir, force_filter_list, 
                    global_min_max, compen_depth=0, compen_ratio=1, image_size = (480,640),
                    unseen=[], 
                    test_unseen=False):  
        """  
        ...
        """  
        self.source, self.target = direction.split("_")
        self.modulus_dir = modulus_dir
        self.modulus_func = {"source_down":np.poly1d(np.loadtxt(os.path.join(self.modulus_dir,f"{self.source}_down.csv"), delimiter=",")), 
                             "source_up":np.poly1d(np.loadtxt(os.path.join(self.modulus_dir,f"{self.source}_up.csv"), delimiter=",")),
                             "target_down":np.poly1d(np.loadtxt(os.path.join(self.modulus_dir,f"{self.target}_down.csv"), delimiter=",")), 
                             "target_up":np.poly1d(np.loadtxt(os.path.join(self.modulus_dir,f"{self.target}_up.csv"), delimiter=","))}
        ...
        self.compen_depth = compen_depth
        self.compen_ratio = compen_ratio
    ...
    def _collect_data(self):  
        ...
        for shape in os.listdir(self.force_dir):  # Process main folders (e.g., sphere_s, cylinder_si)  
            ...
            depth_shape_dir = os.path.join(self.depth_dir, shape)  
            ...
            if os.path.isdir(force_shape_dir) and os.path.isdir(npy_shape_dir):  
                for subfolder in os.listdir(force_shape_dir):  # Process subfolders (e.g., 1, 2, 3)  
                    ...
                    depth_subfolder = os.path.join(depth_shape_dir, subfolder)
                  
                    #depth
                    if os.path.isdir(force_subfolder) and os.path.isdir(img_subfolder):  
                        ft_path = os.path.join(force_subfolder, "ft.csv") 
                        depth_path_down, depth_path_up = os.path.join(depth_subfolder,"inc_z.csv"), os.path.join(depth_subfolder,"dec_z.csv")
                        depth_down, depth_up = np.loadtxt(depth_path_down,delimiter=","), np.loadtxt(depth_path_up,delimiter=",")

                        if self.source == "uskin":
                            depth_down, depth_up= (1.08649-depth_down)/(1.08649+0.0736), (1.08649-depth_up)/(1.08649+0.0736)
                        elif self.source == "tactip":
                            depth_down, depth_up= (4.648126681-depth_down)/(4.648126681+0.049249177), (4.648126681-depth_up)/(4.648126681+0.049249177)
                        elif self.source == 'gelsight':
                            depth_down, depth_up= (1.12310475-depth_down)/(1.12310475+0.07459319), (1.12310475-depth_up)/(1.12310475+0.07459319)

                        #calculate ratio
                        down_source, up_source = self.modulus_func["source_down"](depth_down), self.modulus_func["source_up"](depth_up)
                        down_target, up_target = self.modulus_func["target_down"](depth_down), self.modulus_func["target_up"](depth_up)
                        down_ratio, up_ratio = down_target / down_source,  up_target / up_source
                        material_correct_ratio = np.concatenate([down_ratio,up_ratio])

                        depth_all = np.concatenate([depth_down,depth_up])
                        if self.target == "tactip":
                            if self.source == "uskin":
                                depth_larger_than_threshold = depth_all>self.compen_depth
                                material_correct_ratio = np.where(depth_larger_than_threshold==1,(material_correct_ratio-1)*self.compen_ratio+1,1)
                            elif self.source == "gelsight":
                                depth_larger_than_threshold = depth_all>self.compen_depth
                                material_correct_ratio = np.where(depth_larger_than_threshold==1,(material_correct_ratio-1)*self.compen_ratio+1,1)
                        elif self.target == "gelsight":
                            if self.source == "tactip":
                                depth_larger_than_threshold = depth_all>self.compen_depth
                                material_correct_ratio = np.where(depth_larger_than_threshold==1,(material_correct_ratio-1)*self.compen_ratio+1,1)
                            if self.source == "uskin":
                                depth_larger_than_threshold = depth_all>self.compen_depth
                                material_correct_ratio = np.where(depth_larger_than_threshold==1,(material_correct_ratio-1)*self.compen_ratio+1,1)
                        elif self.target == "uskin":
                            if self.source == "gelsight":
                                depth_larger_than_threshold = depth_all>self.compen_depth
                                material_correct_ratio = np.where(depth_larger_than_threshold==1,(material_correct_ratio-1)*self.compen_ratio+1,1)
                            if self.source == "tactip":
                                depth_larger_than_threshold = depth_all>self.compen_depth
                                material_correct_ratio = np.where(depth_larger_than_threshold==1,(material_correct_ratio-1)*self.compen_ratio+1,1)
                        # print(f"ratio:{material_correct_ratio}")
                        try:
                            forces = pd.read_csv(ft_path, usecols=[0, 1, 2]).values * material_correct_ratio[:, np.newaxis]
                        except Exception as e:
                            print(f"{depth_path_down} is not aligned")
                        # print(f"force:{forces}")
                        if os.path.exists(ft_path) and (not ft_path in self.force_filter):  
                            img_files = sorted([  
...
def create_dataloader(img_dir, force_dir, force_filter_list, global_min_max, args, shuffle=True):  
    ...
    """  
    dataset = SequentialDataset(args.task, force_dir, img_dir, args.modulus, args.src_depth, force_filter_list, global_min_max, compen_depth = args.compen_depth, compen_ratio = args.compen_ratio, unseen=args.unseen, test_unseen=args.test_unseen) 
