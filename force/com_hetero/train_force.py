...
def get_parser():  
    ...
    parser.add_argument('--src_depth', type=str, default="dataset/1")  
    parser.add_argument('--modulus', type=str, default="dataset/1") 
    parser.add_argument('--compen_depth', type=float, default=0) 
    parser.add_argument('--compen_ratio', type=float, default=0.5) 
...
def load_data_infer(args, min_max, shuffle=True, domain="source"):  
    """Load dataset for training or testing."""  
    if domain == "source" :
        img_dir = args.src_img    # xxx/images
        force_dir = args.src_force  # xxx/forces
    ...
    return data_loader_infer.create_dataloader_infer(img_dir, force_dir, force_filter_list, min_max, args, shuffle=shuffle)  
...
def main():  
    ...
    if args.draw_force:  
        ...
        source_data, target_data = load_data_infer(args, min_max_dict, shuffle=False, domain="source"),load_data_infer(args, min_max_dict, shuffle=False,domain="target")

        predict_force(model, min_max_dict, args, save_dir, source_data, "source")
