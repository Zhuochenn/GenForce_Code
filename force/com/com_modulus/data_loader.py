            ...
            if os.path.isdir(force_shape_dir) and os.path.isdir(npy_shape_dir):  
                for subfolder in os.listdir(force_shape_dir):  # Process subfolders (e.g., 1, 2, 3)  
                    ...
                    if os.path.isdir(force_subfolder) and os.path.isdir(img_subfolder):  
                        ...
                        #calculate ratio
                        down_source, up_source = self.modulus_func["source_down"](np.loadtxt(depth_path_down,delimiter=",")), self.modulus_func["source_up"](np.loadtxt(depth_path_up,delimiter=","))
                        down_target, up_target = self.modulus_func["target_down"](np.loadtxt(depth_path_down,delimiter=",")), self.modulus_func["target_up"](np.loadtxt(depth_path_up,delimiter=","))
                        down_ratio, up_ratio = down_target / down_source,  up_target / up_source
                        material_correct_ratio = np.concatenate([down_ratio,up_ratio])

                        depth_all = np.concatenate([np.loadtxt(depth_path_down,delimiter=","),np.loadtxt(depth_path_up,delimiter=",")])
