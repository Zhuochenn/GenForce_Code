...
class SequentialDataset(Dataset):  
    def __init__(self, force_dir, img_dir, force_filter_list, 
                    global_min_max, args, image_size = (480,640),
                    unseen=[], 
                    test_unseen=False):  
        """  
    ...
    def _collect_data(self):  
        ...
        for shape in sorted(os.listdir(self.force_dir)):  # Process main folders (e.g., sphere_s, cylinder_si)  
            ...
            # if shape != self.args.infer_indenter: continue
            if self.test_unseen:
            ...
            if os.path.isdir(force_shape_dir) and os.path.isdir(npy_shape_dir):  
                for subfolder in os.listdir(force_shape_dir):   # Process subfolders (e.g., 1, 2, 3)  
                    # if subfolder != self.args.infer_point: continue
        ...
        # ordered_save_path = os.path.join(self.args.save_dir, 'csv_paths.csv') 
        # save_path_imgs = os.path.join(self.args.save_dir, 'imgs_paths.csv')  

        # df = pd.DataFrame({'csv_path': ordered_csv_paths})  
        # df_imgs = pd.DataFrame({'imgs_path': img_file_path})  

        # df.to_csv(ordered_save_path, index=False)  
        # df_imgs.to_csv(save_path_imgs, index=False,header=False)  

        random.seed(42) 
        random.shuffle(data)

        # Extract and flatten the 'img_files' lists in the shuffled order  
        shuffled_img_files = []  
        for item in data:  
            shuffled_img_files.extend(item["img_files"])  # Flatten the lists  

        # Save the shuffled 'img_files' paths to a CSV file  
        output_csv_path = os.path.join(self.args.save_dir, 'imgs_paths.csv')  
        with open(output_csv_path, mode="w", newline="") as file:  
            writer = csv.writer(file)  
            # writer.writerow(["img_files"])  # Write the header  
            for img_file in shuffled_img_files:  
                writer.writerow([img_file])  # Write each image file path 
        return data  
...
def create_dataloader(img_dir, force_dir, force_filter_list, global_min_max, args, shuffle=True):  
    ...
    """  
    dataset = SequentialDataset(force_dir, img_dir, force_filter_list, global_min_max, args, unseen=args.unseen, test_unseen=args.test_unseen) 
